...
var app = angular.module('viewCustom', ['angularLoad', 'lodAuthorCard']);
...
angular.module("lodAuthorCard", []).filter('escape', function () {
...
}).controller('LodAuthorCardComponentController', ['$http', '$scope', 'lodAuthorCardOptions', function ($http, $scope, lodAuthorCardOptions) {
  var vm = this;
  ...
  $scope.loading = true;
  $http.get(jsonldUrl).then(function (data) {
    ...
    if (creator) return creator['@id'];else throw "No LC Names URI found";
  }).then(function (uri) {
    return $http.get(lodAuthorCardOptions.bibcardApi + uri);
  }).then(function (data) {
    $scope.bibcard = data.data;
  }).catch(function (e) {
    console.warn(e);
  }).finally(function () {
    $scope.loading = false;
