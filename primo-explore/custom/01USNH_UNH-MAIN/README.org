...
* Description
This is the literate configuration for the University of New Hampshire Durham campus libraries' Primo VE customization package.
This document is both the documentation and the actual configuration, in that this document will auto-generate the required JavaScript, S/CSS/ and HTML files.

I write this configuration in Emacs with Org Mode, using =org-babel= to tangle the code blocks.
...
** General
This code block uses NoWEB to structure the ~main.js~ file.
All configuration below will "tangle" to ~main.js~ according to the structure below (or other files as indicated).

*** JavaScript

When using =--browserify=, we use ~main.js~ instead of ~custom.js~.
This file has to include our:
1. package imports,
2. our configuration files,
3. our PRM directives,
4. our modules,
5. and our app constants, components, and values.

LibChat goes after modules but before configuration!
...
*** SCSS
UNH uses SCSS to customize look and feel.
Here are some general SCSS partials for our Main view that don't correlate to any extensions.

**** Partials
***** Magnifying Glass Button and LOAD Button
This changes the magnifying glass and LOAD button to a gray color.
This has to be separated out from the "positive" color configuration;
otherwise, it changes these to green.

#+begin_src scss :tangle ./scss/partials/_button-confirm.scss
/* magnifying glass and load more */
...
.service-form-static-panel
  ...
  color: #010304;
  background-color: rgba(38, 54, 69, 0.1);
}

#+end_src

***** Left Spacer
This centers the search box over the search results when facets are on the left side.
For some reason, this is not done automatically.

#+begin_src scss :tangle ./scss/partials/_left-spacer.scss
...
The External Search package creates a facet in Primo and Primo VE which allows users to repeat their search query in various external search indexes, such as WorldCat and Google Scholar.

First, install via the command line:

#+begin_src shell
cd path/to/view/folder
yarn add --dev @orbis-cascade/primo-explore-external-search
#+end_src

Then add the import statement to ~main.js~

#+begin_src javascript :noweb-ref imports
import "@orbis-cascade/primo-explore-external-search";
#+end_src

And add the package name to the modules in ~main.js~
...
*** Config
We need to configure the links for External Search.
Here, we configure UNH's proxied WorldCat and our proxied Google Scholar.
The rest of the configuration comes from the package instructions.
...
For the WorldCat logo URL, I had to use the relative URL for the /website/, not the GitHub repository file structure!
Hence, ~/discovery/custom/~ instead of ~/primo-explore/custom/~.
I think this has to do with us being Primo VE.

We need to import the configuration variable into ~main.js~
...
And add the variable to our app configuration
...
const Items = [
  {
    ...
    description:
      "brief static html content with an overview on how to use the search box",
  ...
  {},
  {
    ...
    description: `brief description of material types, "scopes", and collections; may have submenus for each of the above`,
...
#+end_src
*** SCSS
#+begin_src scss :tangle ./scss/partials/_libchat.scss
/* positions libchat widget lower on page; visible on mobile screens both orientations */
.lcs_slide_out-r {
  top: 50% !important;
}

/* moves widget higher when clicked so the bottom is visible on mobile screens both orientations */
...
#+end_src
*** SCSS
#+begin_src scss :tangle ./scss/partials/_search-bar-sub-menu.scss
/*
  The reason these are such specific pixels is that they were chosen
  to space correctly around the existing search bar
  Of course if the size of the existing search bar is different of if ExL
  changes it these base values will be meaningless
*/
prm-search-bar {
...
search-bar-sub-menu
  div.layout-align-end-center.layout-row.flex.search-bar-sub-menu {
  align-items: center;
